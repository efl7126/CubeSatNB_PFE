...
cc_tx_cw(const cw_pulse_t *in, size_t len)
{
  ...
  if(activerCW)	// Ne pas transmettre si activerCW=0
  {
	  for(i = 0; i < len; i++) {
		if(in[i].cw_on){
		  cc_tx_cmd (STX);
...
// Fonction test inspiree de celle proposee par TI
int32_t
runTX (const uint8_t *data, size_t size, uint8_t *rec_data,
		       size_t timeout_ms)
{
	uint8_t mode;
	uint8_t bufferByte;
    ...
    cc_tx_spi_write_fifo (data + issue_len, rec_data, issue_len);
    // bufferByte = cc_tx_cmd (STX);
    ...
    // Recevoir l'interruption de fin de paquet
    timeout = 1;
    while (HAL_GetTick () - start_tick < timeout_ms) {
		//    Remove this and the satellite will EXPLODE!
		delay_us(800);
