...
*/
// #include "verification_service.h" Comment Add to remove error
// #include "large_data_service.h" Comment Add to remove error
/*
#include "sensors.h"
...
// Changement pour faire compiler
// struct _comms_data comms_data;
struct _comms_data comms_data = {0};
...
is_tx_enabled()
{
  ...
  return 1;	// Add so tx is always enabled
}
...
comms_routine_dispatcher(comms_tx_job_list_t *tx_jobs)
{
 ...
 /*   rx_sync_flag = 0;
	...
	// Allumer la LED verte pour fins de test
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_SET);	// LED verte
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_RESET);  // LED orange
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_RESET);  // LED rouge

	// Envoi de texte sur le port USB pour DEBUG
	sendOverUSB("tx_cw \r\n");
	...
	// Allumer la LED verte pour fins de test
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET);	// LED verte
	...
	// Allumer la LED rouge pour fins de test
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET);	// LED verte
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_RESET);  // LED orange
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_SET);  // LED rouge

	// Envoi de texte sur le port USB pour DEBUG
	// sendOverUSB("tx_ext_wod \r\n");
    ...
    // ret = comms_ex_wod_tx();
	  ...
	  // Comment Add to remove error
	struct uart_data obc_uart_local = comms_data.obc_uart;


	import_pkt_pfe(OBC_APP_ID, &comms_data.obc_uart);
...
//    export_pkt (OBC_APP_ID, &comms_data.obc_uart);
	...
	HAL_IWDG_Refresh (&hiwdg);
      ...
      {
/*    	// Afficher le nombre d'erreurs TX ou RX avant de ne pas rafraichir le watchdog
    	uint8_t myBuf[100];
    	sprintf(myBuf,"Failed RX: %ld; Failed TX: %ld", comms_stats.rx_failed_cnt, comms_stats.tx_failed_cnt);
    	strcat(myBuf, "\r\n");
    	sendOverUSB(myBuf);*/

    	//TODO
    	// Enlever avant de terminer le projet! Ceci est seulement la pour eviter
    	// que le microcontrolleur reset lorsque les TX ne fonctionnent pas
